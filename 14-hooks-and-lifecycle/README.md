...
Hooks let you run custom code at defined points in the agent lifecycle. Pre-tool, post-tool, session start, session end, before compaction, after compaction — each is a hook event that matched handlers can respond to.

At fire time the runtime picks a **hook event** name (for example `PreToolUse`, `SessionStart`), matches configured handlers, runs them under policy, and merges their outcomes for the caller.

### Two different "registries"

Do not conflate these:

| Registry | Purpose | Scope | Backing code sample |
|----------|---------|-------|---------------------|
| **Event / matcher resolution** | At fire time, **merges** hook definitions from several sources (saved settings, session-scoped hooks from skills or agent frontmatter, plugin-registered matchers, and internal callbacks), **sorts** matchers by priority, and runs every hook whose matcher fits the payload. This is the structural "what hooks exist for this event" graph. | All hook types | [`hook_registry.py`](code-samples/hook_registry.py) — `MultiHandlerHookRegistry` |
| **Pending async subprocess registry** | Command hooks may print a first-line JSON object with an **async** marker and keep running. The runtime **tracks** that shell handle in a dedicated global map: default timeout (order of tens of seconds), periodic checks for a **synchronous** JSON line on stdout, delivery of progress/outcomes, and **finalization** on shutdown. | Long-running shell hooks only | [`async_pending_hooks.py`](code-samples/async_pending_hooks.py) |

> **Tie-in -- Chapter 03 (Permission system):** In interactive mode, **workspace trust** is required before **any** hook runs, so hooks are not a bypass for permission boundaries. Hook handlers can also carry permission results back into the agent loop — see [Chapter 03 -- Permission system](../03-permission-system/README.md).

> **Tie-in -- Chapter 12 (Skills and Plugins):** Swarm and teammate flows plug into the same lifecycle without redefining hook semantics. Plugin hooks re-register when plugins load so the active set swaps atomically with the rest of session lifecycle — see [Chapter 11 -- Multi-agent coordination](../11-multi-agent-coordination/README.md).

---

## 13.1 Hook events and matchers
...
**Wide event surface** — The canonical hook event tuple spans `PreToolUse` through `FileChanged`; SDK schemas stay in sync with it.
...
Each matched hook runs through an **execution backend**. No single backend fits every trust model and latency budget — [`hook_execution_backends.py`](code-samples/hook_execution_backends.py) sketches the dispatch idea.

| Backend | How it runs | Trust model | Latency |
|---------|------------|-------------|---------|
| Command | Shell subprocess, JSON stdin/stdout | Local, auditable | Low |
| HTTP | POST to URL | Network-isolated | Medium |
| Agent | In-process with tool access | Full trust | Variable |

### Command backend

Spawns a subprocess (bash by default, or PowerShell when configured): one JSON payload on **stdin**; **stdout** parsed line-by-line for hook JSON. If the first JSON line declares **async**, the process may stay alive: the runtime registers it in the **pending async** registry, applies a timeout, polls for a later JSON line with the decision, then finalizes. Strict **timeouts** and **output caps** are mandatory. Session paths are passed in environment variables.

### HTTP backend

POSTs the same JSON string to a configured URL, parses the response body as hook JSON, and respects URL allowlists, timeouts, optional SSRF handling, and sandbox network routing when enabled.

### Agent / prompt / callback backends

Agent hooks run in-process with full tool access. Other backends include **prompt** templates, **plugin callbacks**, and **function** hooks backed by session storage.

---

## 13.3 Async pending hooks

Command hooks may print a first-line JSON object with an **async** marker and keep running. The runtime then tracks that shell handle in a dedicated global map:

1. **Default timeout** — order of tens of seconds.
2. **Periodic polling** — checks for a **synchronous** JSON line on stdout.
3. **Progress/outcome delivery** — streamed back to the caller as they arrive.
4. **Finalization on shutdown** — outstanding processes are drained or terminated.
...
See [`async_pending_hooks.py`](code-samples/async_pending_hooks.py) for the pattern.

---

## 13.4 Lifecycle events

A **session** is one conversational agent run from first touch through teardown.

### Session events

**Session hooks** are handlers bound to session-scoped events such as `SessionStart` and `SessionEnd` (and related setup/teardown names in the canonical list).
...
A concise procedural model is in [`stop_hooks_pipeline.py`](code-samples/stop_hooks_pipeline.py); the agent loop may consult stop-related signals (see [Chapter 01 -- Agent loop](../01-agent-loop/README.md)).
