...
### Concrete example: parent spawning a subagent

The following sketch shows the most common shape -- a parent delegates a focused task to a child with restricted tools and a bounded run:

```
Parent agent (main loop, all tools)
  -> spawns "code-review" subagent
     - tools: [Read, Grep, Glob] only
     - working dir: /project/src/
     - max turns: 5
     - returns: summary string to parent
```

The parent continues its own loop after receiving the summary. The child's full transcript is stored on a **sidechain** so it never bloats the parent context window.

> **Tie-in -- Chapter 01 (Agent loop):** The child uses the *same* async-generator pattern described in [Chapter 01](../01-agent-loop/README.md) -- stream output, run tools, append results, check stop conditions. Recovery logic and per-turn limits apply independently inside each nested loop, so a child failure does not corrupt the parent's generator state.

> **Tie-in -- Chapter 04 (System prompt / execution scope):** Each subagent gets its own execution scope as described in [Chapter 04](../04-system-prompt/README.md). The system prompt is **frozen at spawn time**, and any per-agent overrides (allowed directories, environment variables, output-token caps) are isolated from the parent. Changing `max_output_tokens` on a child can alter thinking/budget fields and **break cache parity** with the parent, so fork-style children typically inherit the parent value unchanged.
...
### 10.4 Tool filtering vs exact tools
...
These roles are often split across modules (e.g. an agent **tool** entrypoint, a **runner** coroutine, and a small **fork** helper). Each pattern is described below.

### Nested agent runner

The main long-lived entry point for spawning a child agent.

- **Merge** optional parent prefix with **task messages**
- Choose **filtered** vs **exact** tools
- Apply **session allow rules** when an explicit allowlist is provided
- Build **system prompt** from **override** (frozen) or fresh assembly
- **Clone** file and replacement state when inheriting context
- Stream messages and optionally expose:
  - **`onCacheSafeParams`** -- for periodic forked summaries
  - **`onQueryProgress`** -- liveness while thinking streams omit assistant rows

### Forked worker loop

A shorter helper path for cache-aligned children.

- Run **`query()`** in a loop with **`CacheSafeParams`** identical to the parent
- **Isolate** `toolUseContext`
- Accumulate **usage** back to parent
- Optionally **`skipTranscript`** or **`skipCacheWrite`** for ephemeral work
- Optional **`onMessage`** callback for UI updates

### Implicit fork spawn

Used when policy allows **omitting** agent type.

- Child inherits **full** parent message list and **rendered** system prompt
- **Exact** tool surface and **thinking** config match the parent for prefix parity
- **Boilerplate** in history detects **fork children** to block **recursive** fork at call time

### Fork message builder

Constructs the wire-format messages shared across sibling forks.

- Emit **assistant** row with all **tool_use** blocks
- Emit **user** row with **tool_result** blocks sharing **one** placeholder string
- Append a **per-child directive** text block -- only the directive differs across siblings

### Sidechain recorder

Persists nested turns without polluting the main thread.

- Flag rows with a **sidechain** marker and optional **parent / agent ids** for resume and debugging
- Optional **grouping key** for related nested runs
- Rows are not treated as ordinary main-thread turns
