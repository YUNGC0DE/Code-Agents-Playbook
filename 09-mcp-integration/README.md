...
- **Outcomes before “tools ready”** — A server can be **disabled** (configured but not started), **still connecting**, **connected**, **needs authentication** (remote OAuth not completed—often surfaced with a small **auth helper** in the tool list until the user finishes the flow), or **failed**. Subagents that declare **required** MCP servers may **wait** for pending connections up to a bounded time so they do not start with a false “server missing” error.
...
## From MCP `tools/list` to agent tools (MCPTool pattern)
